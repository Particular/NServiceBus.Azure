...
- `MessageInvisibleTime`: The peek lock system, supported by azure storage queues relies on a period of time that a message becomes locked/invisible after being read. If the processing unit fails to delete the message by the specified time it will reappear on the queue so that another process can retry. This value is defined in milliseconds and defaults to 30000 (30 seconds). It's important to note that currently this value is per message in the batch, and will be multiplied with the batch size to get the effective invisible time. (We consider changing this behavior in the future).
